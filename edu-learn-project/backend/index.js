...
function parseLessonItem(lesson, chapterIndex, lessonIndex) {
  ...
  return chapters
    .map((chapter, chapterIndex) => {
      ...
      const lessons = Array.isArray(chapter?.lessons)
        ? chapter.lessons.map((l, lIdx) => parseLessonItem(l, chapterIndex, lIdx)).filter(Boolean)
        : [];
      return {
...
function checkRegisterTypes(body) {
  if (!body || typeof body !== 'object') return 'Dữ liệu đăng ký không hợp lệ.';
  const fields = ['full_name', 'email', 'phone', 'password'];
  for (const field of fields) {
    if (typeof body[field] !== 'string') return 'Họ tên, email, số điện thoại và mật khẩu phải là chuỗi.';
  }
  return null;
}

function checkRegisterFormats(name, email, phone, password) {
  if (!name || !email || !phone || !password) return 'Vui lòng cung cấp đầy đủ thông tin.';
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return 'Email không đúng định dạng.';
  if (!/^0(?:3|5|7|8|9)\d{8}$/.test(phone)) return 'Số điện thoại phải gồm 10 chữ số và bắt đầu bằng 03, 05, 07, 08 hoặc 09.';
...
function validateRegisterInput(body) {
  const typeError = checkRegisterTypes(body);
  ...
  const formatError = checkRegisterFormats(trimmedName, trimmedEmail, trimmedPhone, body.password);
  if (formatError) return { error: formatError };

  return { full_name: trimmedName, email: trimmedEmail, phone: trimmedPhone, password: body.password, error: null };
}

async function handleRegisterEmailConflict(db, existingEmail) {
  if (existingEmail.role === 'USER') {
    return { status: 400, message: 'Bạn đã có tài khoản.' };
  }
  ...
  return { status: 400, message: 'Email đã tồn tại trên hệ thống.' };
}
...
app.post('/api/auth/register', async (req, res) => {
  ...
  const { full_name, email, phone, password } = validated;

  try {
    const db = await getDatabase();
    const existingEmail = await db.get("SELECT * FROM users WHERE email = ?", [email]);
    if (existingEmail) {
      const conflict = await handleRegisterEmailConflict(db, existingEmail);
...
function createLoginTokenAndPayload(user) {
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    if (user.status === 'blocked' && ['MANAGER', 'STAFF'].includes(user.role)) {
      return res.status(403).json({ message: 'Bạn chưa đăng ký, vui lòng đăng ký' });
    }

    res.json(createLoginTokenAndPayload(user));
  } catch (error) {
...
function buildCourseFilterQuery(query) {
  const { search, category, price_range, sort_by } = query;
  ...
  const orderBy = sortMap[sort_by] || 'created_at DESC';

  return {
    sql: `SELECT * FROM courses WHERE ${whereConditions.join(' AND ')} ORDER BY ${orderBy}`,
    params
  };
}
...
app.get('/api/courses', async (req, res) => {
  try {
    const db = await getDatabase();
    const { sql, params } = buildCourseFilterQuery(req.query);
...
async function syncComboDetailsAndPrice(db, comboId, courseIds) {
  const placeholders = courseIds.map(() => '?').join(',');
  const coursesData = await db.all(`SELECT price, sale_price FROM courses WHERE id IN (${placeholders})`, courseIds);
  const price = coursesData.reduce((sum, c) => sum + (c.sale_price || c.price), 0);

  await db.run(`DELETE FROM combo_details WHERE combo_id = ?`, [comboId]);
  for (const courseId of courseIds) {
    await db.run(`INSERT INTO combo_details (combo_id, course_id) VALUES (?, ?)`, [comboId, courseId]);
  }
  return price;
}

function resolveComboUpdateValues(existing, body, price) {
  const defaultImg = 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=800&q=80';
  const comboImage = body.image === undefined ? existing.image : (body.image || defaultImg);
  const comboStatus = body.status || existing.status;
  const description = body.description || '';
  const salePrice = body.sale_price || null;
  return [body.title, description, comboImage, price, salePrice, comboStatus];
}

app.put('/api/admin/combos/:id', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  const { title, course_ids } = req.body;
  if (!title || !Array.isArray(course_ids) || course_ids.length === 0) {
    return res.status(400).json({ message: 'Tên combo và danh sách khóa học là bắt buộc.' });
  }
  try {
    const db = await getDatabase();
    ...
    const price = await syncComboDetailsAndPrice(db, req.params.id, course_ids);
    const updateValues = resolveComboUpdateValues(existing, req.body, price);
    ...
    const combo = await db.get("SELECT * FROM combos WHERE id = ?", [req.params.id]);
    const comboCourses = await db.all(
      `SELECT c.* FROM courses c JOIN combo_details cd ON c.id = cd.course_id WHERE cd.combo_id = ?`,
      [req.params.id]
    );
    res.json({ ...combo, courses: comboCourses.map(formatCourse) });
  } catch (error) {
    res.status(500).json({ message: 'Lỗi server.', error: error.message });
  }
});
...
async function insertFaqItems(db, list) {
  ...
  for (const [index, faq] of list.entries()) {
    ...
    const id = faq.id || `faq-${Date.now()}-${index}`;
    const order = Number(faq.display_order ?? index);
    const createdAt = faq.created_at || new Date().toISOString();
    await db.run(
      'INSERT INTO faqs (id, question, answer, display_order, created_at) VALUES (?, ?, ?, ?, ?)',
      [id, q, faq.answer, order, createdAt]
    );
  }
}

async function saveFaqSettings(db, settings) {
  if (!settings) return;
  const title = settings.title ? settings.title.trim() : 'Câu hỏi thường gặp';
  const desc = settings.description || '';
  await db.run(
    `INSERT INTO faq_settings (id, title, description, updated_at) VALUES ('faq-main', ?, ?, ?)
     ON CONFLICT(id) DO UPDATE SET title = excluded.title, description = excluded.description, updated_at = excluded.updated_at`,
    [title, desc, new Date().toISOString()]
  );
}

async function saveFaqsContent(db, data) {
  const isArr = Array.isArray(data);
  const list = isArr ? data : (data && data.list ? data.list : []);
  const settings = isArr ? null : (data ? data.settings : null);

  await insertFaqItems(db, list);
  await saveFaqSettings(db, settings);
}

async function saveSingleSectionContent(db, section, data) {
  ...
  if (Object.prototype.hasOwnProperty.call(singleTables, section)) {
    ...
    await db.run(
      `INSERT INTO ${table} (id, title, description, content, updated_at) VALUES (?, ?, ?, ?, ?)
       ON CONFLICT(id) DO UPDATE SET title = excluded.title, description = excluded.description, content = excluded.content, updated_at = excluded.updated_at`,
      [id, data.title.trim(), data.description || '', data.content || '', new Date().toISOString()]
    );
  }
}

async function saveContactSectionContent(db, data) {
  if (!data?.address?.trim() || !data?.phone?.trim() || !data?.email?.trim() || !data?.title?.trim()) {
    throw new Error('Vui lòng nhập đầy đủ thông tin liên hệ.');
  }
  await db.run(
    `INSERT INTO contact_settings (id, title, description, address, phone, email, content, updated_at) VALUES ('contact-main', ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(id) DO UPDATE SET title = excluded.title, description = excluded.description, address = excluded.address, phone = excluded.phone, email = excluded.email, content = excluded.content, updated_at = excluded.updated_at`,
    [data.title.trim(), data.description || '', data.address.trim(), data.phone.trim(), data.email.trim(), data.content || '', new Date().toISOString()]
  );
}
...
app.put('/api/admin/website-content/:section', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  ...
  const { data } = req.body;
  if (!['faqs', 'terms', 'guides', 'introductions', 'contacts'].includes(section)) {
    return res.status(404).json({ message: 'Nhóm nội dung không hợp lệ.' });
  }
  ...
  try {
    ...
    if (section === 'faqs') await saveFaqsContent(db, data);
    else if (section === 'contacts') await saveContactSectionContent(db, data);
    else await saveSingleSectionContent(db, section, data);
    ...
    res.status(500).json({ message: 'Không thể lưu nội dung.', error: error.message });
  }
});
...
// ================= MY NOTIFICATIONS =================
async function getMyOrderNotifications(db, userId) {
...
function getAffiliateStatusNotification(affRecord) {
  ...
  if (affRecord.status === 'rejected' || affRecord.status === 'terminated') {
    return {
...
function mapAffiliateOrders(affNotifs) {
  return affNotifs.map(an => {
...
function mapAffiliateWithdrawals(myWithdrawals) {
  const titleMap = {
    completed: 'Yêu cầu rút tiền thành công',
    rejected: 'Yêu cầu rút tiền bị từ chối',
    pending: 'Yêu cầu rút tiền đang chờ xử lý'
  };
  const msgMap = {
    completed: w => `Yêu cầu rút tiền mã ${w.id} trị giá ${w.amount.toLocaleString('vi-VN')}đ đã được chuyển khoản thành công.`,
    rejected: w => `Yêu cầu rút tiền mã ${w.id} của bạn đã bị từ chối. Vui lòng kiểm tra thông tin ngân hàng.`,
    pending: w => `Yêu cầu rút tiền mã ${w.id} trị giá ${w.amount.toLocaleString('vi-VN')}đ đã gửi thành công và đang chờ admin phê duyệt.`
  };
  return myWithdrawals.filter(w => titleMap[w.status]).map(w => ({
    id: `withdrawal-${w.status}-${w.id}`,
    type: 'withdrawal-status',
    title: titleMap[w.status],
    message: msgMap[w.status](w),
    time: w.updated_at || w.created_at,
    link: '/tai-khoan?tab=affiliate'
  }));
}

async function getAffiliateActivityNotifications(db, affId) {
  ...
  const clicks = affClicks.map(clk => ({
    id: `aff-click-${clk.id}`,
    type: 'referral-click',
  ...
  return [...mapAffiliateOrders(affNotifs), ...clicks, ...mapAffiliateWithdrawals(myWithdrawals)];
}
...
app.get('/api/my-notifications', authenticateToken, checkUserStatus, async (req, res) => {
  try {
    ...
    let notifications = await getMyOrderNotifications(db, userId);
    ...
    if (affRecord) {
      const statusNotif = getAffiliateStatusNotification(affRecord);
      if (statusNotif) notifications.push(statusNotif);

      if (affRecord.status === 'approved') {
        const activityNotifs = await getAffiliateActivityNotifications(db, affRecord.id);
        notifications = notifications.concat(activityNotifs);
      }
    }
...
app.post('/api/orders/upload-proof', authenticateToken, checkUserStatus, async (req, res) => {
  try {
    ...
    await db.run("UPDATE orders SET payment_proof = ? WHERE id = ?", [proof_image, order_id]);
...
app.get('/api/orders/:id', authenticateToken, checkUserStatus, async (req, res) => {
  try {
    ...
    const user = await db.get("SELECT full_name, email, phone FROM users WHERE id = ?", [order.user_id]);
...
async function fetchAdminNotificationLists(db) {
  ...
  const notifications = [];
  pendingOrders.forEach(o => notifications.push({
    id: `order-pending-${o.id}`, type: 'order', title: 'Đơn hàng mới chờ duyệt',
    message: `Đơn hàng ${o.id} trị giá ${o.total.toLocaleString('vi-VN')}đ đang chờ phê duyệt.`,
    time: o.created_at, link: '/admin/orders'
  }));
  pendingAffiliates.forEach(a => notifications.push({
    id: `affiliate-pending-${a.id}`, type: 'affiliate', title: 'Đăng ký đối tác mới',
    message: `Tài khoản ${a.full_name} vừa gửi yêu cầu tham gia Affiliate.`,
    time: a.created_at, link: '/admin/affiliates'
  }));
  recentUsers.forEach(u => notifications.push({
    id: `user-new-${u.id}`, type: 'user', title: 'Người dùng mới đăng ký',
    message: `Khách hàng ${u.full_name} đã đăng ký tài khoản thành công.`,
    time: u.created_at, link: '/admin/users'
  }));
  pendingWithdrawals.forEach(w => notifications.push({
    id: `withdrawal-pending-${w.id}`, type: 'withdrawal', title: 'Yêu cầu rút tiền mới',
    message: `Đối tác ${w.full_name} đã yêu cầu rút tiền với số tiền ${w.amount.toLocaleString('vi-VN')}đ.`,
    time: w.created_at, link: '/admin/withdrawals'
  }));
...
app.get('/api/admin/notifications', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  try {
    const db = await getDatabase();
    const notifications = await fetchAdminNotificationLists(db);
    notifications.sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime());
    res.json(notifications);
  } catch (error) {
    res.status(500).json({ message: 'Lỗi server khi lấy thông báo.', error: error.message });
...
async function handleAffiliateOrderCompleted(db, orderId) {
  await db.run("UPDATE affiliate_revenues SET status = 'approved' WHERE order_id = ?", [orderId]);
  const revenues = await db.all("SELECT * FROM affiliate_revenues WHERE order_id = ?", [orderId]);
...
async function updateOrderAndAffiliateStatus(db, orderId, order, status, newPaymentStatus) {
  await db.run('UPDATE orders SET status = ?, payment_status = ? WHERE id = ?', [status, newPaymentStatus, orderId]);
  if (status === 'completed') {
    if (order.status !== 'completed') {
      await handleAffiliateOrderCompleted(db, orderId);
    }
    return;
  }
  if (status === 'cancelled') {
    await db.run("UPDATE affiliate_revenues SET status = 'cancelled' WHERE order_id = ?", [orderId]);
  }
}
...
app.put('/api/admin/orders/:id/status', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  ...
  try {
    ...
    await updateOrderAndAffiliateStatus(db, req.params.id, order, status, newPaymentStatus);

    await db.exec('COMMIT');
    res.json({ message: 'Đã cập nhật trạng thái đơn hàng.', status, payment_status: newPaymentStatus });
  } catch (error) {
    if (db) {
      try { await db.exec('ROLLBACK'); } catch { /* ignore rollback error */ }
    }
...
app.patch('/api/admin/orders/:id/payment-status', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  ...
  try {
    ...
    await db.run('UPDATE orders SET payment_status = ?, status = ? WHERE id = ?', [payment_status, newOrderStatus, req.params.id]);

    if (payment_status === 'da_thanh_toan' && order.status !== 'completed') {
      await handleAffiliateOrderCompleted(db, req.params.id);
    }

    await db.exec('COMMIT');
    res.json({ message: 'Đã cập nhật trạng thái thanh toán.', payment_status, status: newOrderStatus });
  } catch (error) {
    if (db) {
      try { await db.exec('ROLLBACK'); } catch { /* ignore rollback error */ }
    }
...
async function fetchMonthlyDashboardStats(db, targetMonth) {
  const monthlyRevenue = await db.get(
    "SELECT SUM(total) as val FROM orders WHERE status = 'completed' AND strftime('%Y-%m', created_at) = ?",
    [targetMonth]
  );
  ...
  const monthlySuccessOrders = await db.get(
    "SELECT COUNT(*) as val FROM orders WHERE status = 'completed' AND strftime('%Y-%m', created_at) = ?",
    [targetMonth]
  );
  const monthlyCancelledOrders = await db.get(
    "SELECT COUNT(*) as val FROM orders WHERE status = 'cancelled' AND strftime('%Y-%m', created_at) = ?",
    [targetMonth]
  );
  const monthlyStudents = await db.get(
    "SELECT COUNT(*) as val FROM users WHERE role = 'USER' AND strftime('%Y-%m', created_at) = ?",
    [targetMonth]
  );
  const monthlyCoursesSold = await db.get(
    `SELECT COUNT(*) as val FROM order_details od JOIN orders o ON od.order_id = o.id WHERE o.status = 'completed' AND strftime('%Y-%m', o.created_at) = ?`,
    [targetMonth]
  );
...
app.get('/api/admin/stats/monthly', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  ...
  try {
    const db = await getDatabase();
    const stats = await fetchMonthlyDashboardStats(db, targetMonth);
    res.json(stats);
  } catch (error) {
...
async function fetchCumulativeDashboardStats(db, targetMonth) {
...
app.get('/api/admin/stats/monthly-cumulative', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  ...
  try {
    const db = await getDatabase();
    const stats = await fetchCumulativeDashboardStats(db, targetMonth);
    res.json(stats);
  } catch (error) {
...
function validateAdminAccountPayload(body) {
  const { full_name, email, password, role, phone } = body || {};
  if (!full_name || !email || !password || !role || !phone) {
    return { error: 'Vui lòng điền đầy đủ thông tin (Họ tên, Email, Mật khẩu, Vai trò, Số điện thoại).' };
  }
  const phoneDigits = phone.replace(/\D/g, '');
  if (!/^(03|05|07|08|09|02[0-9])[0-9]{8}$/.test(phoneDigits)) {
    return { error: 'Số điện thoại không đúng định dạng Việt Nam.' };
  }
  const normalizedEmail = email.trim().toLowerCase();
  if (!normalizedEmail.endsWith('@gmail.com')) {
    return { error: 'Email phải có đuôi @gmail.com.' };
  }
  return { full_name, email: normalizedEmail, phone: phoneDigits, password, role, error: null };
}
...
app.post('/api/admin/accounts', authenticateToken, checkUserStatus, requireRole(['MANAGER']), async (req, res) => {
  ...
  try {
    const db = await getDatabase();
    const existing = await db.get("SELECT id FROM users WHERE email = ?", [validated.email]);
    if (existing) {
    ...
    await db.run(
      ...
      [`admin-${Date.now()}`, validated.full_name, validated.email, validated.phone, hash, validated.role, 'active', 1, now]
    );
...
async function updateAdminAccountFields(db, id, body) {
  const { full_name, email, password, role, phone } = body;
  if (full_name) await db.run("UPDATE users SET full_name = ? WHERE id = ?", [full_name, id]);
  if (email) {
    const normalizedEmail = email.trim().toLowerCase();
    if (!normalizedEmail.endsWith('@gmail.com')) throw new Error('Email phải có đuôi @gmail.com.');
    await db.run("UPDATE users SET email = ? WHERE id = ?", [normalizedEmail, id]);
  }
  if (password) {
  ...
  if (phone !== undefined) {
    ...
    if (!/^(03|05|07|08|09|02[0-9])[0-9]{8}$/.test(digits)) throw new Error('Số điện thoại không đúng định dạng Việt Nam.');
    await db.run("UPDATE users SET phone = ? WHERE id = ?", [digits, id]);
  }
}
...
app.put('/api/admin/accounts/:id', authenticateToken, checkUserStatus, requireRole(['MANAGER']), async (req, res) => {
  try {
    ...
    await updateAdminAccountFields(db, req.params.id, req.body);
    res.json({ message: 'Cập nhật tài khoản quản trị thành công.' });
  } catch (error) {
    res.status(400).json({ message: error.message || 'Lỗi cập nhật tài khoản.' });
  }
});
  ...
  try {
    ...
    await db.run(
      ...
      [
        ...
        JSON.stringify(parseCourseCurriculum(content)), courseInstructor, status || 'published', new Date().toISOString()
      ]
    );
...
function fallbackField(val, existingVal, defaultVal = null) {
  if (val === undefined) return existingVal;
  return val || defaultVal;
}

function buildCourseUpdateParams(body, existing, id) {
  const defaultImg = 'https://images.unsplash.com/photo-1461749280684-dccba630e2f6?w=800&q=80';
  const img = fallbackField(body.image, existing.image, defaultImg);
  const instructor = body.instructor ? body.instructor.trim() : 'Nguyễn Văn A';
  const desc = fallbackField(body.description, existing.description, '');
...
app.put('/api/admin/courses/:id', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  ...
  try {
    ...
    if (!existing) return res.status(404).json({ message: 'Không tìm thấy khóa học.' });

    const params = buildCourseUpdateParams(req.body, existing, req.params.id);
    await db.run(
      `UPDATE courses SET title = ?, description = ?, image = ?, price = ?, sale_price = ?, category_id = ?, content_html = ?, highlights = ?, curriculum = ?, instructor = ?, status = ? WHERE id = ?`,
      params
    );
...
app.delete('/api/admin/courses/:id', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  try {
    ...
    if (result.changes === 0) return res.status(404).json({ message: 'Không tìm thấy khóa học.' });
...
// ================= AFFILIATE ROUTES =================
async function registerAffiliateInDb(db, body, userId, existing) {
  const { full_name, email, phone, bank_name, bank_account, address, dob } = body;
  const now = new Date().toISOString();
  let ctvCode = existing?.ctv_code || (await generateUniqueCtvCode(db));
  const clientUrl = process.env.CLIENT_URL || 'http://localhost:3000';
  const affiliateLink = `${clientUrl}?ref=${ctvCode}`;

  if (existing) {
    await db.run(
      `UPDATE affiliates SET full_name = ?, email = ?, phone = ?, bank_name = ?, bank_account = ?, address = ?, dob = ?, status = 'pending', ctv_code = ?, affiliate_link = ?, created_at = ? WHERE user_id = ?`,
      [full_name, email, phone, bank_name, bank_account, address, dob, ctvCode, affiliateLink, now, userId]
    );
  } else {
    ...
    await db.run(
      `INSERT INTO affiliates (id, user_id, full_name, email, phone, bank_name, bank_account, address, dob, status, ctv_code, ma_ctv, affiliate_link, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [affiliateId, userId, full_name, email, phone, bank_name, bank_account, address, dob, 'pending', ctvCode, ctvCode, affiliateLink, now]
    );
  }
}

function validateAffiliateRegistration(body) {
  const fields = ['full_name', 'email', 'phone', 'bank_name', 'bank_account', 'address', 'dob'];
  for (const f of fields) {
    if (!body || !body[f]) return 'Vui lòng điền đầy đủ thông tin đăng ký.';
  }
  return null;
}

function canAffiliateReapply(existing) {
  if (!existing) return true;
  return existing.status === 'rejected' || existing.status === 'terminated';
}

app.post('/api/affiliates/register', authenticateToken, checkUserStatus, async (req, res) => {
  const validationError = validateAffiliateRegistration(req.body);
  if (validationError) {
    return res.status(400).json({ message: validationError });
  }
  ...
  try {
    ...
    if (!canAffiliateReapply(existing)) {
      return res.status(400).json({ message: 'Bạn đã đăng ký chương trình affiliate rồi.' });
    }

    await registerAffiliateInDb(db, req.body, req.user.id, existing);
    const msg = existing ? 'Đăng ký lại thành công, vui lòng chờ xét duyệt.' : 'Đăng ký thành công, vui lòng chờ xét duyệt.';
...
async function fetchAffiliateReportStats(db, affiliateId, startDate, endDate) {
  let dateFilterSql = "";
  const dateParams = [];
  ...
  const clicks = await db.all(
    `SELECT * FROM affiliate_clicks WHERE affiliate_id = ?${dateFilterSql} ORDER BY created_at DESC`,
    [affiliateId, ...dateParams]
  );
  ...
  return {
    stats: {
      ...
      totalOrders: approvedRevenues.length,
      totalCommission: approvedRevenues.reduce((sum, r) => sum + r.commission_amount, 0),
      totalRevenue: approvedRevenues.reduce((sum, r) => sum + r.order_total, 0)
    },
    clicks,
    revenues
  };
...
app.get('/api/affiliate/report', authenticateToken, checkUserStatus, async (req, res) => {
  try {
    ...
    if (!affiliate) return res.status(404).json({ message: "Không tìm thấy thông tin đối tác Affiliate." });

    const data = await fetchAffiliateReportStats(db, affiliate.id, req.query.startDate, req.query.endDate);
    res.json(data);
  } catch (error) {
    res.status(500).json({ message: 'Lỗi server.', error: error.message });
  }
...
async function syncDriveUserForAffiliate(db, affiliate, status, id) {
  ...
  if (status === 'approved') {
    ...
    await db.run("UPDATE affiliates SET affiliate_email = ? WHERE id = ?", [driveEmail, id]);

    const driveUserExists = await db.get("SELECT id FROM users WHERE email = ?", [driveEmail]);
    if (!driveUserExists) {
      await db.run(
        "INSERT INTO users (id, full_name, email, password, role, status, must_change_password, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        [`u-aff-${Date.now()}`, affiliate.full_name, driveEmail, passwordToUse, 'AFFILIATE', 'active', 0, new Date().toISOString()]
      );
    } else {
      await db.run("UPDATE users SET password = ?, status = 'active', must_change_password = 0 WHERE email = ?", [passwordToUse, driveEmail]);
    }
...
app.put('/api/admin/affiliates/:id/status', authenticateToken, checkUserStatus, requireRole(['MANAGER', 'STAFF']), async (req, res) => {
  ...
  try {
    ...
    if (!affiliate) return res.status(404).json({ message: 'Không tìm thấy yêu cầu affiliate.' });

    await db.run("UPDATE affiliates SET status = ? WHERE id = ?", [status, req.params.id]);
    await syncDriveUserForAffiliate(db, affiliate, status, req.params.id);
    res.json({ message: 'Cập nhật trạng thái thành công.' });
  } catch (error) {
    res.status(500).json({ message: 'Lỗi server.', error: error.message });
  }
...
async function checkProfileConflicts(db, phoneDigits, email, userId) {
  if (phoneDigits) {
    const existingPhone = await db.get("SELECT id FROM users WHERE phone = ? AND id != ?", [phoneDigits, userId]);
    if (existingPhone) return 'Số điện thoại đã tồn tại trên hệ thống.';
  }
  const existingEmail = await db.get("SELECT id FROM users WHERE email = ? AND id != ?", [email.trim(), userId]);
  if (existingEmail) return 'Email đã được sử dụng bởi tài khoản khác.';
  return null;
}

async function checkProfilePasswordChange(db, body, userRecord, userId) {
  const { old_password, new_password } = body;
  if (!new_password) return null;
  if (!old_password) return 'Vui lòng cung cấp mật khẩu cũ để thay đổi mật khẩu.';
  if (new_password.length < 6) return 'Mật khẩu mới phải có ít nhất 6 ký tự.';

  const match = await bcrypt.compare(old_password, userRecord.password);
  if (!match) return 'Mật khẩu cũ không chính xác.';
...
function validateProfileFields(fullName, email, phoneDigits) {
  if (!fullName || !fullName.trim()) return 'Họ và tên không được để trống.';
  if (!email || !email.trim()) return 'Email không được để trống.';
  if (phoneDigits && !/^(03|05|07|08|09|02[0-9])[0-9]{8}$/.test(phoneDigits)) {
    return 'Số điện thoại không đúng định dạng Việt Nam (10 chữ số, bắt đầu bằng 03, 05, 07, 08, 09 hoặc 02).';
  }
  return null;
}

async function validateAndCheckProfile(db, body, userId) {
  const { full_name, phone, email } = body;
  const phoneDigits = phone ? phone.replace(/\D/g, '') : '';
  
  const fieldErr = validateProfileFields(full_name, email, phoneDigits);
  if (fieldErr) return { error: fieldErr };

  const conflictErr = await checkProfileConflicts(db, phoneDigits, email, userId);
  if (conflictErr) return { error: conflictErr };

  const userRecord = await db.get("SELECT * FROM users WHERE id = ?", [userId]);
  if (!userRecord) return { error: 'Không tìm thấy thông tin tài khoản.', notFound: true };

  const passErr = await checkProfilePasswordChange(db, body, userRecord, userId);
  if (passErr) return { error: passErr };

  await db.run(
    "UPDATE users SET full_name = ?, phone = ?, email = ? WHERE id = ?",
    [full_name.trim(), phoneDigits || null, email.trim(), userId]
  );
  return { error: null };
}
...
app.put('/api/users/profile', authenticateToken, checkUserStatus, async (req, res) => {
  try {
    const db = await getDatabase();
    const result = await validateAndCheckProfile(db, req.body, req.user.id);
    if (result.error) {
      return res.status(result.notFound ? 404 : 400).json({ message: result.error });
    }
  ...
  try {
    ...
    const statistics = await db.all(`
      SELECT
        ...
        a.created_at,
        COALESCE(SUM(CASE WHEN substr(ar.created_at, 1, 7) = ? AND ar.status IN ('approved', 'paid') THEN ar.commission_amount ELSE 0 END), 0) AS monthly_commission,
        COALESCE(SUM(CASE WHEN ar.status IN ('approved', 'paid') THEN ar.commission_amount ELSE 0 END), 0) AS total_commission
      FROM affiliates a
      LEFT JOIN affiliate_revenues ar ON ar.affiliate_id = a.id
      WHERE a.status = 'approved' AND COALESCE(NULLIF(a.ma_ctv, ''), NULLIF(a.ctv_code, '')) IS NOT NULL
...
// ================= WITHDRAWAL REQUESTS =================
async function checkAffiliateAvailableBalance(db, affiliateId) {
  ...
  return Number(commission.total) - Number(withdrawn.total);
}

async function insertAffiliateWithdrawal(db, reqBody, affiliate, amount) {
  const withdrawalId = `wd-${Date.now()}`;
  const bankName = reqBody.bank_name || affiliate.bank_name;
  const bankAccount = reqBody.bank_account || affiliate.bank_account;
...
app.post('/api/affiliate/withdrawals', authenticateToken, checkUserStatus, async (req, res) => {
  ...
  try {
    ...
    const affiliate = await db.get(
      "SELECT * FROM affiliates WHERE (user_id = ? OR affiliate_email = ? OR email = ?) AND status = 'approved'",
      [req.user.id, req.user.email, req.user.email]
    );
    if (!affiliate) return res.status(404).json({ message: 'Không tìm thấy thông tin affiliate.' });

    const available = await checkAffiliateAvailableBalance(db, affiliate.id);
    if (amount > available) return res.status(400).json({ message: 'Số dư khả dụng không đủ.' });

    const withdrawalId = await insertAffiliateWithdrawal(db, req.body, affiliate, amount);
...
function buildResetPasswordEmailContent(siteName, fromEmail, resetLink, user) {
  ...
  return {
    ...
    html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Đặt lại mật khẩu</title></head>
<body style="font-family: Arial, sans-serif; background-color: #f3f4f6; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    ...
    <div style="padding: 30px; text-align: center;">
      <h2 style="color: #111827; margin: 0 0 12px 0;">Đặt lại mật khẩu</h2>
      <p style="color: #6b7280; margin: 0 0 16px 0;">Xin chào <strong>${nameOrEmail}</strong>, nhấn nút bên dưới để tạo mật khẩu mới (hiệu lực trong 1 giờ).</p>
      <a href="${resetLink}" target="_blank" style="display: inline-block; padding: 14px 32px; background: #2563eb; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: bold;">🔐 Đặt lại mật khẩu</a>
      <p style="color: #9ca3af; margin: 24px 0 0 0; font-size: 13px;">Nếu không yêu cầu, vui lòng bỏ qua email này.</p>
    </div>
  </div>
</body></html>`,
...
async function createPasswordResetToken(db, userId) {
  ...
  await db.run('UPDATE password_reset_tokens SET used = 1 WHERE user_id = ?', [userId]);
  await db.run(
    'INSERT INTO password_reset_tokens (id, user_id, token, expires_at, used, created_at) VALUES (?, ?, ?, ?, 0, ?)',
    [tokenId, userId, token, expiresAt, new Date().toISOString()]
  );
  return token;
}
...
app.post('/api/forgot-password', async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ message: 'Chưa có tài khoản với email này, vui lòng đăng ký' });
    ...
    const mailOptions = { ...buildResetPasswordEmailContent(siteName, fromEmail, resetLink, user), to: email };
