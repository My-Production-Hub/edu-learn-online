...
function createDbTransport(config, email, pass) {
...
async function getTransporter() {
  ...
  const currentPassword = config?.password?.trim() || null;
  ...
  cachedTransporter = (currentEmail && currentPassword)
    ? createDbTransport(config, currentEmail, currentPassword)
...
function buildEmailHeader(siteName) {
  return `
...
function buildCustomerInfoSection(userName, userEmail, orderId) {
  return `
...
function buildOrderTableSection(itemsHtml, totalFormatted, paymentMethod) {
  return `
...
function buildOrderEmailHtml({ siteName, fromEmail, userName, userEmail, orderId, orderDetails }) {
  ...
  return `
    <!DOCTYPE html>
    <html>
    <head><meta charset="UTF-8"><title>Cảm ơn bạn đã đặt hàng</title></head>
    <body style="font-family: Arial, sans-serif; background-color: #f3f4f6; margin: 0; padding: 0;">
      <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
        ${buildEmailHeader(siteName)}
        <div style="padding: 30px; background-color: #ffffff;">
          ${buildCustomerInfoSection(userName, userEmail, orderId)}
          ${buildOrderTableSection(itemsHtml, totalFormatted, paymentMethod)}
        </div>
...
function buildOrderEmailText({ siteName, fromEmail, userName, userEmail, orderId, orderDetails }) {
  ...
  const itemsText = items.map(item => `- ${item.product_name || item.title}: ${item.price ? Number(item.price).toLocaleString('vi-VN') + 'đ' : 'N/A'}`).join('\n') || 'Không có chi tiết';
  const totalFormatted = orderDetails.total ? Number(orderDetails.total).toLocaleString('vi-VN') + 'đ' : 'N/A';
  const paymentMethod = orderDetails.payment_method || 'Chuyển khoản ngân hàng';
